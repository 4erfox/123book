...
export async function renderAssetsPanel(container) {
    ...
    listContainer.innerHTML = assets.map(asset => `
        <div style="display: flex; align-items: center; gap: 12px; padding: 10px; border-bottom: 0.5px solid ${t.border};">
            ...
            <div style="display: flex; gap: 5px;">
                ...
                <button class="set-favicon-btn" data-path="${asset.path}" style="padding: 4px 8px; background: rgba(34, 197, 94, 0.1); border: 0.5px solid rgba(34, 197, 94, 0.3); border-radius: 5px; cursor: pointer; color: #22c55e; font-size: 9px; font-family: ${t.mono};">
                    Favicon
                </button>
    ...
    document.querySelectorAll('.set-favicon-btn').forEach(btn => {
        btn.addEventListener('click', async () => {
            ...
            try {
                ...
                reader.onload = async (e) => {
                    ...
                    await bridge.uploadFavicon(base64, blob.type);
                    showToast('Favicon установлен', 'success');
                    updateFaviconOnPage();
                };
                reader.readAsDataURL(blob);
            } catch (err) {
                showToast('Ошибка установки favicon', 'error');
            }
        });
    });
